...
Eligible BCBSVT press coverage also receives a five-label sentiment question from `src/rubrics/sentiment-v2.json`. It starts with the existing tracker rules and matching storyline notes. The aligned profile supplies up to 16 paired human sentiment examples and more explicit label distinctions. In enforce mode, sentiment requires a valid answer with confidence of at least 0.7. Other coverage is not scored. Jev returns a label rather than a written rationale, so an applied Jev score clears the previous model's rationale. Jev also returns the odds for all five labels, and those become a 0-100 `sentimentScore` (0 negative, 50 neutral, 100 positive; the probability-weighted position on the five-point scale), published only beside the Jev label it came from. Sentiment applies to eligible coverage on both sides of the enforcement boundary; inclusion is still enforced only after it. Eligible coverage without stored odds (older articles Jev never scored, and entries cached before the odds were kept) is asked again for sentiment alone, using whatever the per-run cap leaves after new articles, and a cached include decision is kept. Jev likewise writes no inclusion rationale. When it adds an article that the first review left out, the "Why it is here" note keeps Gemini's description of the article and names the scope rated highest, with its confidence, for example "National ACA enrollment changes. Fits U.S. health coverage, insurance, or policy news (93% confidence)." The note does not name the model. A Gemini line that states a rejection is dropped. Notes are rebuilt from the saved first-review description on every run. Additions cached before scope scores were stored are asked again for those scores alone, within the per-run request cap, and the cached include decision is kept. Both inclusion and sentiment thresholds remain uncalibrated starting points.
